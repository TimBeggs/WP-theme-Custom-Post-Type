...
<article <?php post_class(); ?> id="post-<?php the_ID(); ?>">
	...
	$image_url = ! post_password_required() ? get_the_post_thumbnail_url( get_the_ID(), 'sophelle-fullscreen' ) : '';
				...
				<div class="cover-color-overlay color-accent<?php echo esc_attr( $color_overlay_classes ); ?>"<?php echo $color_overlay_style; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- We need to double check this, but for now, we want to pass PHPCS ;) ?>></div>
					...
					<header class="entry-header has-text-align-center">
						<div class="entry-header-inner section-inner medium">
							 ...
							 */
							$show_categories = apply_filters( 'sophelle_show_categories_in_entry_header', true );

							if ( true === $show_categories && has_category() ) {
								?>

								<div class="entry-categories">
									<span class="screen-reader-text"><?php _e( 'Categories', 'sophelle' ); ?></span>
							...
							if ( is_page() ) {
								...
								<div class="to-the-content-wrapper">
									...
									<a href="#post-inner" class="to-the-content fill-children-current-color">
										<?php sophelle_the_theme_svg( 'arrow-down' ); ?>
										<div class="screen-reader-text"><?php _e( 'Scroll Down', 'sophelle' ); ?></div>
									</a><!-- .to-the-content -->
								...
								sophelle_the_post_meta( get_the_ID(), 'single-top' );
	...
	<div class="post-inner" id="post-inner">
		...
		wp_link_pages(
			array(
				'before'      => '<nav class="post-nav-links bg-light-background" aria-label="' . esc_attr__( 'Page', 'sophelle' ) . '"><span class="label">' . __( 'Pages:', 'sophelle' ) . '</span>',
				'after'       => '</nav>',
		...
		sophelle_the_post_meta( get_the_ID(), 'single-bottom' );
